import { safeFindPilotLines } from '@/lib/safeQuery';
import Header from '@/components/Header';
import Footer from '@/components/Footer';
import BookingForm from '@/components/BookingForm';
import { getSiteGlobalConfig } from '@/lib/siteConfig';

export const dynamic = 'force-dynamic';

export default async function BookingPage() {
  const lines = await safeFindPilotLines('desc');
  const config = await getSiteGlobalConfig();

  return (
    <>
      <Header />
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px' }}>
        <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#1E3A8A', marginBottom: '8px' }}>{config.booking.title}</h1>
        <p style={{ color: '#6B7280', marginBottom: '32px' }}>{config.booking.subtitle}</p>
        <BookingForm lines={lines.map(l => ({ id: l.id, name: l.name, region: l.region, status: l.status, specs: l.specs, capacity: l.capacity }))} />
      </div>
      <Footer />
    </>
  );
}
